...
%%% AKJ: Dictionary = Dictionary.*repmat(sign(Dictionary(1,:)),size(Dictionary,1),1); % multiply in the sign of the first element.
...
if (length(relevantDataIndices)<1) %(length(relevantDataIndices)==0)
    ...
    betterDictionaryElement = betterDictionaryElement; %AKJ - .*sign(betterDictionaryElement(1));
    CoefMatrix(j,:) = 0;
    NewVectorAdded = 1;
    return;
end
